...
if __name__ == '__main__':
    ...
    mtext = boolean2.modify_states( text=text, turnon=['miR484'] )
    avgs = run( text=mtext, repeat=REPEAT, nodes=NODES, steps=STEPS) 
    data.append( avgs )
    ...
    mtext = boolean2.modify_states( text=text, turnon=['let7b'] )
    avgs = run( text=mtext, repeat=REPEAT, nodes=NODES, steps=STEPS) 
    data.append( avgs )
    ...
    mtext = boolean2.modify_states( text=text, turnoff=['miR484'] )
    avgs = run( text=mtext, repeat=REPEAT, nodes=NODES, steps=STEPS) 
    data.append( avgs )
    ...
    mtext = boolean2.modify_states( text=text, turnoff=['let7b'] )
    avgs = run( text=mtext, repeat=REPEAT, nodes=NODES, steps=STEPS) 
    data.append( avgs )
