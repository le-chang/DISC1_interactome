...
if __name__ == '__main__':
    ...
    mtext = boolean2.modify_states( text=text, turnoff=['ACTB'] )
    avgs = run( text=mtext, repeat=REPEAT, nodes=NODES, steps=STEPS) 
    data.append( avgs )

    mtext = boolean2.modify_states( text=text, turnoff=['AKT1'] )
    avgs = run( text=mtext, repeat=REPEAT, nodes=NODES, steps=STEPS) 
    data.append( avgs )
    ...
    mtext = boolean2.modify_states( text=text, turnoff=['CCDC88A'] )
    avgs = run( text=mtext, repeat=REPEAT, nodes=NODES, steps=STEPS) 
    data.append( avgs )
